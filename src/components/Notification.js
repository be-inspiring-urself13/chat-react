...
function Notification() {
    ...
    return(
        ...
        <div className="searchBox">
            <input type="text" placeholder="Search by Name..." id="searchInput" onChange={(e) => setQuery(e.target.value)} />
            <FaSearch color="black" style={{marginRight:"20px", fontSize:"22px"}} />
        </div>
                ...
                userData.filter((user) => user.username.toLowerCase().includes(query.toLowerCase())).map((user) => {
                    return(
                        <div className="notification" key={user.id}>
                            <img className="profilePic" src={user.userprofile} alt="pic" />
